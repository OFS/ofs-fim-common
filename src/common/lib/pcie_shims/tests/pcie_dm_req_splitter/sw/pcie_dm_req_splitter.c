...
uint32_t hash_update(uint32_t cur_hash32, uint32_t num_buckets,
                     const uint8_t *start_ptr, const uint8_t *end_ptr)
{
...
int main(int argc, char *argv[])
{
    ...
    uint64_t bus_byte_width;
    fpgaReadMMIO64(accel_handle, 0, 8 * 8, &bus_byte_width);
    printf("Bus byte width: %ld\n\n", bus_byte_width);
    ...
    // via CSRs.
    //
    uint32_t num_hash_buckets = bus_byte_width / 4;
    uint32_t *cur_hash32 = malloc(sizeof(uint32_t) * num_hash_buckets);
    assert(NULL != cur_hash32);
    for (int h_idx = 0; h_idx < num_hash_buckets; h_idx += 1)
    {
    ...
    for (int i = 0; i < 100000; i += 1)
    {
        ...
        for (int c = 0; c < NUM_MEM_REQS; c += 1)
        {
            ...
            {
                cur_hash32[h_idx] =
                    hash_update(cur_hash32[h_idx], num_hash_buckets,
                                (const uint8_t*)&rd_buf[history_rd_start[c] + h_idx * 4],
                                (const uint8_t*)&rd_buf[history_rd_start[c] + history_len[c]]);
            }
        }

        // Compare hashes. Each hash checks 64 bits of the data bus.
        for (int h_idx = 0; h_idx < bus_byte_width/8; h_idx += 1)
        {
            // Pick the HW hash bucket. Adding 4 to the index skips over the
            // PCIe SS TLP header that occupies the first 32 bytes of every packet.
            fpgaWriteMMIO64(accel_handle, 0, 4 * 8, h_idx + 4);
            fpgaReadMMIO64(accel_handle, 0, 9 * 8, &v);

            // Combine two adjacent 32 bit hashes into a single 64 bit value to
            // match the MMIO behavior.
