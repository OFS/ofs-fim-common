...
usage()
{
   echo "Usage: gen_sim_files.sh <target(s)> <device (optional)> <family (optional)>"
...
PROJECT_DIR="${PROJECT_PARENT}"/syn/syn_top
...
(cd "${OFS_ROOTDIR}"; \
 ALLOW_PROJ_IN_OFS_ROOTDIR=sim ./ofs-common/scripts/common/syn/build_top.sh --stage setup ${OFS_TARGET} "${PROJECT_PARENT}")
# UVM scripts expect qip_gen to point to the tree where qsys-generate is run
(cd "${SIM_SETUP_DIR}"; rm -rf qip_gen; ln -s "${PROJECT_NAME}/syn/ip_lib" qip_gen)
echo
...
# converts the paths to be relative to PROJECT_PARENT.
rm -f "${SIM_SETUP_DIR}"/generated_ip_flist.f
(cd "${PROJECT_DIR}"
 for ip in $(grep ^set_global_assignment "${PROJECT_DIR}"/project_ip_for_sim.tcl | sed -e 's/.*_FILE *//'); do
     realpath --relative-to="${OFS_ROOTDIR}" "${ip}" >> "${SIM_SETUP_DIR}"/generated_ip_flist.f
 done
)

rm -f "${SIM_SETUP_DIR}"/generated_rtl_flist*
(cd "${PROJECT_DIR}"
 ...
 for inc_path in $(cat project_sources_rtl_incdirs.f | colrm 1 8); do
     ...
     realpath --relative-to="${SIM_SETUP_DIR}" "${inc_path}" >> "${SIM_SETUP_DIR}"/generated_rtl_flist_incdirs.f
 done
