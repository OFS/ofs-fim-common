...
do
    ...
    if [ -f "$SIM_SETUP_DIR/ip_flist.sh" ] ; then
    	cp "$SIM_SETUP_DIR/ip_flist.sh" "$SIM_SETUP_DIR/ip_flist_old.sh"
    fi

    if [ -f "$SIM_SETUP_DIR/ip_flist.f" ] ; then
    	cp "$SIM_SETUP_DIR/ip_flist.f" "$SIM_SETUP_DIR/ip_flist_old.f"
    fi
    ...
    if [ ! -f "$SIM_SETUP_DIR/ip_flist.sh" ] || [ -w "$SIM_SETUP_DIR/ip_flist.sh" ] ; then
    	...
    	echo "Warning: ip_flist.sh is not write-able, please check the file permission."
    fi
