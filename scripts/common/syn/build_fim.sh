...
if [[ $OFS_BOARD_CORE =~ n600 ]]; then
   ...
   if grep -q "ENABLE_MULTI_AUTHORITY\s\+ON" ${WORK_SYN_TOP_PATH}/${Q_PROJECT}.qsf; then
      if [ -f "${OFS_ROOTDIR}/ofs-common/syn/common/setup/vab_sw/u-boot-spl-dtb.hex" ]; then
         echo "    Using VAB-enabled firmware in build: ${OFS_ROOTDIR}/ofs-common/syn/common/setup/vab_sw/u-boot-spl-dtb.hex"
         cp ${OFS_ROOTDIR}/common/setup/vab_sw/u-boot-spl-dtb.hex ${WORK_SYN_TOP_PATH}/
      else
         echo "    Error: VAB-enabled firmware file: ${OFS_ROOTDIR}/ofs-common/syn/common/setup/vab_sw/u-boot-spl-dtb.hex could not be found."
      fi
   else
      if [ -f "${OFS_ROOTDIR}/ofs-common/syn/common/setup/non_vab_sw/u-boot-spl-dtb.hex" ]; then
         echo "    Using non-VAB-enabled firmware in build: ${OFS_ROOTDIR}/ofs-common/syn/common/setup/non_vab_sw/u-boot-spl-dtb.hex"
         cp ${OFS_ROOTDIR}/ofs-common/syn/common/setup/non_vab_sw/u-boot-spl-dtb.hex ${WORK_SYN_TOP_PATH}/
      else
         echo "    Error: non-VAB-enabled firmware file: ${OFS_ROOTDIR}/ofs-common/syn/common/setup/non_vab_sw/u-boot-spl-dtb.hex could not be found."
      fi
   fi
fi
