...
proc emit_ip_cfg {ofile_name ip_name} {
    ...
    # Instantiate the LED status and speed ports
    # divide port leds across 2 available LED pins
    set led_count 0
    set num_led_pins [expr $total_ports / 2]
    puts $of ""
    puts $of ""
    for {set j 0 } { $j < $total_ports } { set j [expr $j + $lanes_per_port]  } {
        if { $port_enabled_arr($j) == 1 } {
              if { $j % $num_led_pins == 0 } {
                 puts $of ""
                 puts  $of "`define INST_ALL_LED_$led_count\(led_type, led_idx, operator) \\"
                 incr led_count
              }
              puts $of "    `INST_LED(``led_type``,$j,``led_idx``, operator) \\"
          } 
    } 
