...
#################################################
######   optional user pre-finish script

# Hook for user-supplied script, invoked only if the environment
# variable OFS_PRE_FINISH_SCRIPT is defined. OFS build environment
# variables pointing to the project, revision build directory, etc.
# remain present.
if [ ! -z ${OFS_PRE_FINISH_SCRIPT} ]; then
    echo "Running OFS_PRE_FINISH_SCRIPT: ${OFS_PRE_FINISH_SCRIPT}"
    # Run from the project directory
    (cd ${WORK_SYN_TOP_PATH}; ${OFS_PRE_FINISH_SCRIPT} pre_finish)
    chk_exit_code "${OFS_PRE_FINISH_SCRIPT} pre_finish"
fi
...
#################################################
######   optional user post-finish script

# Hook for user-supplied script, invoked only if the environment
# variable OFS_POST_FINISH_SCRIPT is defined. OFS build environment
# variables pointing to the project, revision build directory, etc.
# remain present.
if [ ! -z ${OFS_POST_FINISH_SCRIPT} ]; then
    echo "Running OFS_POST_FINISH_SCRIPT: ${OFS_POST_FINISH_SCRIPT}"
    # Run from the project directory
    (cd ${WORK_SYN_TOP_PATH}; ${OFS_POST_FINISH_SCRIPT} post_finish)
    chk_exit_code "${OFS_POST_FINISH_SCRIPT} post_finish"
fi
