...
#    Agilex Stratix 10 Partial Reconfiguration Flow Script
#
#    This template, once configured for your design, can be used to invoke the
#    partial reconfiguration flow for an Agilex or Stratix 10 design. 
#    Revision names for each PR implementation along with the partitions they implement
#    can be setup in <FUTURE_IMPROVEMENT: add script name for PR setup>
...
proc my_export_partition {partition snapshot qdb_filename } {
	execute_module -dont_export_assignments -tool cdb -args "--export_block $partition --snapshot $snapshot --file $qdb_filename --include_sdc_entity_in_partition"
}
...
proc initialize {skip_base_check} {
	...
	if {($skip_base_check == 0)} {
		if { [catch {project_open $PROJECT_NAME -rev $BASE_REVISION_NAME -force} msg] } {
			puts $msg
		...
		if {[lsearch -exact [list "STRATIX 10" "AGILEX"] [string toupper [get_dstr_string -family $value]]] == -1} {
			post_message -type error "The current family $value ([get_dstr_string -family $value]) is not supported by the s10_partial_reconfig flow script. This script only supports families: Stratix 10, Agilex."
			qexit -error
		}
...
proc print_pr_project_info {} {
	...
	puts "Partial Reconfiguration Flow"
	puts "-------------------------------------------------------------------------------"
...
proc compile_base_revision { {update_mif 1} } {
	global PROJECT_NAME
	global BASE_REVISION_NAME
    ...
    if { [info exists ::env(POST_IPGENERATE_BASE_FLOW_TCL_PATH)] } {
	    if {[file exists "$::env(POST_IPGENERATE_BASE_FLOW_TCL_PATH)/post_ipgenerate_base_flow.tcl"]} {
		    post_message -type info "Preparing to source $::env(POST_IPGENERATE_BASE_FLOW_TCL_PATH)/post_ipgenerate_base_flow.tcl."
		    source "$::env(POST_IPGENERATE_BASE_FLOW_TCL_PATH)/post_ipgenerate_base_flow.tcl"
	    }
    }
	...
	execute_module -dont_export_assignments -tool fit
        if { $update_mif == 1 } {
	        ...
	        #exec python ./update_fme_ifc_id.py .. $BASE_REVISION_NAME
	        if { [info exists ::env(UPDATE_FME_IFC_ID_PY_FILE)] } {
                exec python3 $::env(UPDATE_FME_IFC_ID_PY_FILE) . $BASE_REVISION_NAME
...
proc main {} {
	...
	set available_options {
		...
		{ disable_mif_gen "Disable MIF update flow" }
	}
...
if {($::quartus(nameofexecutable) == "quartus") || ($::quartus(nameofexecutable) == "quartus_pro") || ($::quartus(nameofexecutable) == "qpro")} {
	...
	post_message -type error "The PR compile flow script can only be run from the Quartus GUI or quartus_sh."
}
