...
# If a Quartus project is defined, emit the project macros to a file next to the
# the PIM configuration .ini file. Change the name suffix from .ini to .macros.
# The PIM setup script will find the macros and may use them to enable interfaces.
if [ ! -z "${WORK_SYN_TOP_PATH}" ]; then
   (cd "${WORK_SYN_TOP_PATH}"
    quartus_sh -t "${OFS_ROOTDIR}"/ofs-common/scripts/common/syn/emit_project_macros.tcl --project=${Q_PROJECT} --revision=${Q_REVISION} --mode=txt --output="${PIM_INI_FILE%.*}".macros
   )
fi
