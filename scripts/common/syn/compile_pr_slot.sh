...
if [ ! -z ${ENA_PR_COMPILE} ]; then
    if [ ${ENA_PR_COMPILE} == "1" ]; then
    ( cd ${WORK_SYN_TOP_PATH} && \
            if quartus_sh -t ${FLOW_TCL_FILE} -setup_script ${SETUP_TCL_FILE} -impl ${Q_PR_REVISION}; then
                quartus_sh -t ${USER_CLOCK_FREQS_COMPUTE_TCL_FILE} --project=${Q_PROJECT} --revision=${Q_PR_REVISION}
                quartus_sta -t ${REPORT_TIMING_TCL_FILE} --project=${Q_PROJECT} --revision=${Q_PR_REVISION}
            fi
    )
    fi
fi
...
# grab PR UUID to include in gbs file
PR_UUID=$(cat ${WORK_SYN_TOP_PATH}/fme-ifc-id.txt)
# check PR_UUID
if [ ${PR_UUID} == "" ]; then
    echo "PR UUID code not in fme-ifc-id.txt"
    exit 1
fi

if [ ! -z ${ENA_GBS_GENERATION} ]; then
    echo "Creating GBS file: ${WORK_PR_GBS_FILE}"
    echo "for PR_UUID: ${PR_UUID}"
    packager create-gbs \
            --gbs=${WORK_PR_GBS_FILE} \
            --afu-json=${WORK_PR_JSON_FILE} \
            --rbf=${WORK_PR_RBF_FILE} \
            --set-value interface-uuid:${PR_UUID}
    chk_exit_code "packager create-gbs --gbs="${WORK_PR_GBS_FILE}" --afu-json=${WORK_PR_JSON_FILE} --rbf=${WORK_PR_RBF_FILE} --set-value interface-uuid:${PR_UUID}" 
fi

# PACSign gbs file and create _unsigned.gbs (for open boards - no root key hash)
...
echo ""
echo "Compile work directory:     ${WORK_DIR}/syn/${OFS_PROJECT}/${OFS_FIM}/${OFS_BOARD}/syn_top"
echo "Compile artifact directory: ${WORK_DIR}/syn/${OFS_PROJECT}/${OFS_FIM}/${OFS_BOARD}/syn_top/output_files"
...
if [ ! -s ${WORK_DIR}/syn/${OFS_PROJECT}/${OFS_FIM}/${OFS_BOARD}/syn_top/output_files/timing_report/clocks.sta.fail.summary ]; then
    echo "***        Timing Passed!"
else
    echo "***        Timing FAILED!!!"
fi
